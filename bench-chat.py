#!/usr/bin/env python3
"""bench.py adapted for servers exposing only /v1/chat/completions (mlx_vlm.server, llama-server).

Bonsai 2 variant: identical to Weschera/Qwen3.8-27B-oMLX-MTP-Mac bench-chat.py plus an explicit
chat_template_kwargs.enable_thinking flag (BENCH_THINKING=off default) so the numbers are comparable
to that repo's thinking-off ladder.
...
THINKING = os.environ.get("BENCH_THINKING", "off") == "on"  # Bonsai 2 defaults to thinking ON; the ladder is measured thinking OFF
